...
class SqlServerSchemaProvider {
    async getSchema(connectionString) {
      ...
      `);
            const primaryKeySet = new Set(primaryKeysResult.recordset.map((row) => `${row.table_schema}.${row.table_name}.${row.column_name}`));
            const foreignKeyColumnSet = new Set(foreignKeysResult.recordset.map((row) => `${row.from_schema}.${row.from_table}.${row.from_column}`));
            ...
            for (const row of columnsResult.recordset) {
                ...
                const column = {
                    ...
                    isPrimaryKey: primaryKeySet.has(`${row.table_schema}.${row.table_name}.${row.column_name}`),
                    isForeignKey: foreignKeyColumnSet.has(`${row.table_schema}.${row.table_name}.${row.column_name}`),
                };
                tableMap.get(tableKey).columns.push(column);
            }
            const relationships = foreignKeysResult.recordset.map((row) => ({
...
exports.SqlServerSchemaProvider = SqlServerSchemaProvider;
//# sourceMappingURL=SqlServerSchemaProvider.js.map